...
function updateTimerDisplay() {
  ...
  const joinedCounterElement = document.getElementById("joinedCounter");
  
  if (timerElement) {
    timerElement.innerHTML = `<span style="color: ${timeLeft < 30 ? '#ff5555' : '#fff'}">Time Left: ${timeLeft}s</span>`;
  }
  
  if (joinedCounterElement) {
    joinedCounterElement.innerHTML = 
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  setTimeout(() => {
    startTimer();
    updateTimerDisplay(); // Ensure the timer display is updated immediately
